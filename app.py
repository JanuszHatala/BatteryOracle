...
def build_single_report_markdown(report, filter_mode, kpis, diagnosis_text, gen_timestamp=None):
    ...
    md.append("\n---\n*Exported from Battery Oracle on " + datetime.now().strftime("%Y-%m-%d %H:%M") + "*")
    return "\n".join(md)

def build_comparative_markdown(reports, filter_mode, analysis_text, gen_timestamp=None):
...
def execute_ai_action(action_type: str, payload: dict, progress_bar, status_box, curr_cfg: dict):
    ...
    if action_type == "single_diagnosis":
        ...
        if payload["filter_mode"] == "full":
            db.update_report_analysis(payload["rep_id"], new_diag, rep.get("reasoning_trace", ""), updated_at=now_ts)
            rep["initial_analysis"] = new_diag
            rep["analysis_updated_at"] = now_ts
        else:
            db.set_setting(payload["diag_cache_key"], new_diag)
            db.set_setting(f"{payload['diag_cache_key']}_ts", now_ts)
        ...
        db.set_setting(f"{payload['cmp_cache_key']}_ts", now_ts)
    ...
    if not all_reports:
        ...
        with tab_single_report:
            ...
            if filter_mode == "full":
                ...
                diag_ts = active_report.get("analysis_updated_at") or active_report.get("created_at") or active_report.get("timestamp_str", "")
            else:
                cached_filtered = db.get_setting(diag_cache_key)
                cached_ts = db.get_setting(f"{diag_cache_key}_ts")
                if cached_filtered:
                    active_diagnosis = cached_filtered
                    diag_subtitle = f"🎯 Targeted Scope Diagnosis: {filter_mode_choice}"
                    diag_ts = cached_ts or active_report.get("analysis_updated_at") or active_report.get("created_at", "")
                else:
            ...
            with c_diag_act2:
                if active_diagnosis:
                    md_content = build_single_report_markdown(active_report, filter_mode, kpis, active_diagnosis, gen_timestamp=diag_ts)
                    st.download_button(
            ...
            with tab_comp_report:
                ...
                with c_comp_act2:
                    if combined_text and not combined_text.startswith("⚠️"):
                        comp_md = build_comparative_markdown(compared_reports, merged_filter_choice, combined_text, gen_timestamp=gen_timestamp)
                        st.download_button(
    ...
    if view_mode == "Single Profile Audit":
        ...
        if current_profile:
            ...
            with tab_prof_audit:
                ...
                prof_gen_ts = current_profile.get("ai_analysis_updated_at") or current_profile.get("updated_at") or current_profile.get("created_at", "")
                ...
                with c_pact2:
                    if p_analysis and prof_gen_ts:
                        clean_ts = str(prof_gen_ts).replace("T", " ")[:19]
                        st.caption(f"🕒 **Report Generated:** `{clean_ts}` &nbsp;•&nbsp; Grounded in full device inventory & AppOps.")
                    else:
                ...
                if p_analysis:
                    render_ai_block(p_analysis, key_suffix=f"profile_{current_profile['id']}", gen_timestamp=prof_gen_ts)
                    if p_trace:
        ...
        if profile_a and profile_b:
            ...
            cmp_ts = db.get_setting(f"{cmp_cache_key}_ts")
            if not cmp_ts and cached_cmp_analysis:
                cmp_ts = profile_b.get("updated_at") or profile_a.get("updated_at", "")
            ...
            with c_act2:
                if cached_cmp_analysis and cmp_ts:
                    clean_ts = str(cmp_ts).replace("T", " ")[:19]
                    st.caption(f"🕒 **Report Generated:** `{clean_ts}` &nbsp;•&nbsp; Cached evaluation grounded in profile deltas & system policies.")
                elif cached_cmp_analysis:
            ...
            if cached_cmp_analysis:
                render_ai_block(cached_cmp_analysis, key_suffix=f"cmp_{p_a_id}_{p_b_id}", gen_timestamp=cmp_ts)
                st.divider()
